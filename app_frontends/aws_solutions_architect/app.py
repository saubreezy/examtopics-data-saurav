...
# Page configuration
st.set_page_config(
    page_title="AWS SAA-C03 Exam Prep",
    page_icon="📚",
    layout="wide",
...
def load_all_questions() -> List[Dict[str, Any]]:
    ...
    json_files = sorted(
        base_path.glob("AWS-Certified-Solutions-Architect---Associate-SAA-C03_*.json"),
...
def main():
    st.title("📚 AWS Certified Solutions Architect - Associate (SAA-C03) Exam Prep")
    st.markdown("---")
    
    # Initialize session state
    if 'questions' not in st.session_state:
        with st.spinner("Loading exam questions from 205 JSON files..."):

...
# Page configuration
st.set_page_config(
    page_title="Databricks Data Engineer Associate Exam Prep",
    page_icon="📚",
    layout="wide",
...
def load_all_questions() -> List[Dict[str, Any]]:
    """Load and parse all Databricks Data Engineer Associate JSON files."""
    base_path = Path(__file__).parent.parent.parent / "Databricks"
    all_questions = []
    
    # Find all Databricks Data Engineer Associate files
    json_files = sorted(
        base_path.glob("Certified-Data-Engineer-Associate_*.json"),
...
def main():
    st.title("📚 Databricks Certified Data Engineer Associate Exam Prep")
    st.markdown("---")
